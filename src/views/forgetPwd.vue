    ...
    <div class="app-container">
        <div class="container-wrapper">
            <div class="form-wrapper">
                ...
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" disabled v-if="!submitDisabled">重置密码</mt-button>
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" v-if="submitDisabled">重置密码</mt-button>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import { sendForgotPasswordCode, forgotPassword } from '../api/api';
    ...
    export default {
        ...
        methods: {
            ...
            handleSubmit: function() {
                ...
                let forgotPasswordParam = { mobile: this.mobile, code: this.code, new_password : this.password };

                Indicator.open({ text: '正在提交...', spinnerType: 'fading-circle' });

                forgotPassword(forgotPasswordParam).then(res => {
                    Indicator.close();
                    ...
                    if(code !== 0) {
                        ...
                        Toast({ message: '密码重置成功，请重新登录！', position: 'bottom', duration: 2000 });
            ...
            sendCode: function() {
                ...
                let sendForgotPasswordCodeParam = { mobile: this.mobile };

                Indicator.open({ text: '正在发送...', spinnerType: 'fading-circle' });

                sendForgotPasswordCode(sendForgotPasswordCodeParam).then(res => {
                    Indicator.close();
