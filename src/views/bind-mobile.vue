    ...
    <div class="app-container">
        <div class="container-wrapper">
            <div class="form-wrapper">
                ...
                <mt-field label="密码" v-model="password" placeholder="输入至少6位的密码" :type="!showPwd ? 'password' : 'text'" class="mint-field-icon" v-if="!modify" @input="checkSubmit">
                ...
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" disabled v-if="!submitDisabled && !modify">绑定</mt-button>
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" v-if="submitDisabled && !modify">绑定</mt-button>
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" disabled v-if="!submitDisabled && modify">重新绑定</mt-button>
                <mt-button type="primary" @click.native="handleSubmit" class="btn-submit" v-bind:class="buttonSkin" v-if="submitDisabled && modify">重新绑定</mt-button>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import { sendChangeMobileCode, bindMobile, changeMobile } from '../api/api';
        ...
        methods: {
            ...
            handleSubmit: function() {
                if(this.modify) {
                    ...
                    let changeMobileParam = { mobile: this.mobile, code: this.code, password: this.password };

                    Indicator.open({ text: '正在绑定...', spinnerType: 'fading-circle' });

                    changeMobile(changeMobileParam).then(res => {
                        Indicator.close();
                        ...
                        if(code !== 0) {
                            ...
                            Toast({ message: '绑定成功！', position: 'bottom', duration: 2000 });

                            localStorage.setItem('mobile', this.mobile);

                            this.$router.push({ path: '/bindmobilesucc' });
                        }
                    });
                } else {
                    // 绑定
                    ...
                    let bindMobileParam = { mobile: this.mobile, code: this.code, password: this.password };

                    Indicator.open({ text: '正在绑定...', spinnerType: 'fading-circle' });

                    bindMobile(bindMobileParam).then(res => {
                        Indicator.close();
            ...
            sendCode: function() {
                ...
                let sendChangeMobileCodeParam = { mobile: this.mobile };

                Indicator.open({ text: '正在发送...', spinnerType: 'fading-circle' });

                sendChangeMobileCode(sendChangeMobileCodeParam).then(res => {
                    Indicator.close();
